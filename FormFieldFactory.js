 ...
 */
/**
 * MultiSelectField - Based on MultiSelectWithOtherField but without "Other" functionality
 */
class MultiSelectField extends BaseField {
    ...
    render() {
        ...
        // Add options to select element (NO "other" option)
        this.options.forEach(option => {
            const optionElement = document.createElement('option');
            optionElement.value = option.id;
            optionElement.textContent = option.name;
        ...
        // Add individual options (NO "other" option)
        this.options.forEach(option => {
            const customOption = document.createElement('div');
            customOption.className = 'custom-option';
            customOption.setAttribute('data-value', option.id);
    ...
    selectMainOption(option) {
        ...
        this.value = this.selectedValues; // Simple array, no "other" handling
        this.hideError();
        this.handleChange();
    }
    ...
    toggleSelectAll() {
        ...
        if (allSelected) {
            ...
            this.selectedValues = this.options.map(opt => opt.id); // All main options, no "other"
        }

        this.updateDisplayText();
        this.value = this.selectedValues; // Simple array
        this.handleChange();
    }
    ...
    /**
     * FIXED: getValue() method based on MultiSelectWithOtherField but without "other" handling
     */
    getValue() {
        if (this.selectedValues.length === 0) {
            return {
    ...
    /**
     * FIXED: setValue() method based on MultiSelectWithOtherField but simplified
     */
    setValue(values) {
    ...
    /**
     * ADDED: getDisplayValue method for consistency
     */
    getDisplayValue() {
        const value = this.getValue();
...
/**
 * ============================================================
 * BIRTHDATE PICKER FIELD - Popup Calendrier
 * ============================================================
 * Un sélecteur de date de naissance avec popup calendrier
 * - Champ d'affichage cliquable
 * - Calendrier en popup
 * - Se ferme après sélection
 * ============================================================
 */
...
class BirthdatePickerField extends BaseField {
    ...
    render() {
        ...
        // Main display field (clickable input)
        const inputWrapper = document.createElement('div');
        inputWrapper.className = 'birthdate-input-wrapper';
        inputWrapper.innerHTML = `
            <div class="birthdate-display" tabindex="0">
                <span class="birthdate-display-text placeholder">${this.getPlaceholder()}</span>
                <div class="birthdate-display-icon">
                    ${this.factory.SVG_ICONS?.CALENDAR || '<svg viewBox="0 0 24 24" width="20" height="20"><path fill="currentColor" d="M19 4h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11zM9 11H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2z"/></svg>'}
                </div>
            </div>
        `;
    ...
    updateDisplayText() {
        if (this.state.selectedDate) {
            this.displayText.textContent = this.formatDisplayDate(this.state.selectedDate);
            this.displayText.classList.remove('placeholder');
        } else {
            this.displayText.textContent = this.getPlaceholder();
