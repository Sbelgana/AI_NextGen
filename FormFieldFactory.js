...
class OptionsStepperField extends BaseField {
    constructor(factory, config) {
        ...
        this.max = config.max || this.options.length - 1;
        this.step = config.step || 1;
        this.currentIndex = this.options.findIndex(opt => opt.value === this.value) || 0;
    }
    ...
    render() {
        ...
        stepperContainer.className = 'options-stepper-container';

        this.decrementBtn = document.createElement('button');
        this.decrementBtn.type = 'button';
        this.decrementBtn.className = 'stepper-btn decrement';
        this.decrementBtn.innerHTML = SVGIcons.get('MINUS');
        
        this.valueDisplay = document.createElement('div');
        this.valueDisplay.className = 'stepper-value';
        
        this.incrementBtn = document.createElement('button');
        this.incrementBtn.type = 'button';
        this.incrementBtn.className = 'stepper-btn increment';
        this.incrementBtn.innerHTML = SVGIcons.get('PLUS');

        const decrementHandler = () => this.decrement();
        const incrementHandler = () => this.increment();
        ...
        stepperContainer.appendChild(this.valueDisplay);
    ...
    updateDisplay() {
        if (this.valueDisplay) {
            ...
            this.valueDisplay.textContent = currentOption ? currentOption.label : '';
        }

        if (this.decrementBtn) {
            this.decrementBtn.disabled = this.currentIndex <= this.min;
        }

        if (this.incrementBtn) {
            this.incrementBtn.disabled = this.currentIndex >= this.max;
        }
    }
    ...
    updateValue() {
        const currentOption = this.options[this.currentIndex];
        if (currentOption) {
            this.setState({ value: currentOption.value });
            this.updateDisplay();
            this.handleChange();
        }
    }
    ...
    getValue() {
        const currentOption = this.options[this.currentIndex];
        return currentOption ? currentOption.value : this.state.value;
    }
...
// Placeholder classes for missing field types (should be implemented based on requirements)
class YesNoField extends BaseField {
    render() {
        ...
        const buttonContainer = document.createElement('div');
        buttonContainer.className = 'yesno-buttons';
        
        this.yesButton = document.createElement('button');
        this.yesButton.type = 'button';
        this.yesButton.className = 'btn-yesno btn-yes';
        this.yesButton.textContent = this.factory.getText('yes');
        
        this.noButton = document.createElement('button');
        this.noButton.type = 'button';
        this.noButton.className = 'btn-yesno btn-no';
        this.noButton.textContent = this.factory.getText('no');
        
        const yesHandler = () => this.selectValue(true);
        const noHandler = () => this.selectValue(false);
        
        this.yesButton.addEventListener('click', yesHandler);
        this.noButton.addEventListener('click', noHandler);
        
        this.eventListeners.push(
            { element: this.yesButton, event: 'click', handler: yesHandler },
            { element: this.noButton, event: 'click', handler: noHandler }
        );
    ...
    updateButtonStates() {
        if (this.yesButton && this.noButton) {
            this.yesButton.classList.toggle('active', this.state.value === true);
            this.noButton.classList.toggle('active', this.state.value === false);
        }
    }
    ...
    setValue(value) {
        this.setState({ value: value === true || value === 'true' || value === 'yes' });
        this.updateButtonStates();
    }
}
