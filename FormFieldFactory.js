...
class FieldValueFormatter {
    ...
    extractStructuredValue(fieldConfig, value) {
        if (!this.shouldDisplayValue(value)) {
            return { displayValue: '', rawValue: value, hasSubFields: false };
        }
...
class BaseDataTransformer {
    ...
    transform(originalFormValues) {
        console.log('BaseDataTransformer: Starting transformation...', { originalFormValues });
...
class CreatForm {
    ...
    handleFieldChange = (name, value) => {
        ...
        this.logger.info(`Field ${name} changed:`, { value, formatted: this.formatValue({}, value) });
...
// ============================================================================
// REUSABLE CAL.COM BASE UTILITY CLASS
...
class CalComBaseUtility {
    ...
    async handleRescheduling(formData, config) {
        ...
        try {
            ...
            this.sendToVoiceflow(config, 'reschedule_error', { error: error.message });
            throw error;
        }
    }
    ...
    /**
     * Create standardized submission data structure
     */
    ...
    /**
     * Set API key (useful for dynamic API key setting)
     */
    setApiKey(apiKey) {
        this.apiKey = apiKey;
    }
    ...
    /**
     * Set log prefix (useful for different extensions)
     */
    setLogPrefix(prefix) {
