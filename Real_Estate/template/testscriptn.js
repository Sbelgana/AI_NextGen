...
const SharedPropertyTypes = {
   Residential: {
      en: ["Condo", "Loft / Studio", "Single-story", "Cottage", "Mobile Home", "Multi-level", "Two-story", "Detached", "Semi-detached", "Row House"],
      fr: ["Condo", "Loft / Studio", "Plain-pied", "Chalet", "Maison mobile", "À Paliers multiples", "À étages", "Détaché", "Jumelé", "En rangée"]
   },
   Multiplex: {
...
function generateAirtableFormula(input) {
   const {
      cityName = [], typeHouse = [], bedrooms = 0, bathrooms = 0, priceMax = 0, priceMin = 0, parkingIndoor = "", car = 0, swimmingPool = ""
   } = input;
   const conditions = [];
...
const SellingExtension = {
   ...
   render: ({ trace, element }) => {
      ...
      // Create translation mapping from English to French
      const PropertyTypeTranslation = {};
      Object.entries(SharedPropertyTypes).forEach(([category, translations]) => {
      ...
      const createPropertyTypeRadios = (category, types) => {
         ...
         return `
          <div>
            ...
            const value = categoryTypes.fr[index]; // Always use French value
            return `
      ...
      formContainer.addEventListener("submit", (event) => {
         ...
         window.voiceflow.chat.interact({
            ...
            payload: {
               ...
               city,
               postalCode,
               propertyType, // This will now be the French value regardless of interface language
               yearBuild,
               area,
               roomsNumber,
               bedroomsNumber,
      ...
      formContainer.querySelector("#garage").addEventListener("change", (event) => {
         ...
         if (event.target.checked) {
            ...
            carsField.required = true;
         } else {
            carsField.style.display = "none";
            carsField.required = false;
            carsField.value = "";
         }
      });
...
const PropertySearchExtension = {
   ...
   render: ({
      ...
      const createCityCheckboxes = (category, cities) => `
        ...
        `;
      // Modify the createPropertyTypeCheckboxes function to use French values:
    ...
    <div>
        ...
        <div class="collapse-content">
            ...
            ${types.map((type) => {
         const frenchValue = PropertyTypeTranslation[type] || type;
         return `<div class="checkbox-item">
                    <input type="checkbox" class="property-checkbox" data-category="${category}" 
                        id="type-${type}" 
                        name="property-types" 
                        value="${isEnglish ? frenchValue : type}">
                    <label for="type-${type}">${type}</label>
                </div>`;
      }).join("")}
        </div>
