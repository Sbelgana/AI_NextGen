...
const ImageExtension = {
   ...
   render: ({ trace }) => {
      ...
      // Extract images from trace payload
      const { images } = trace.payload;
      if (!images) {
         console.error('No images provided in trace payload');
         document.getElementById('error-container').textContent = 'Error: No images provided';
         return;
      }
      ...
      try {
         imageList = images.split(',').map(url => url.trim());
         if (imageList.length === 0) {
            console.error('No valid images found in payload');
            document.getElementById('error-container').textContent = 'Error: No valid images found';
            return;
         }
      } catch (error) {
         console.error('Error parsing image list:', error);
         document.getElementById('error-container').textContent = 'Error parsing image list';
         return;
      }
      ...
      // Create image modal and thumbs-up button
      ...
      // Append modal to chatbot element
