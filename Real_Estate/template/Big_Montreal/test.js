...
const PropertySearchExtension = {
    ...
    render: ({ trace, element }) => {
        ...
        const PropertyTypes = Object.fromEntries(
			Object.entries(PropertyTypeMappings[langCode]).map(([label, sharedKey]) => [label, SharedPropertyCategories[sharedKey][langCode]])
        );

        const PropertyTypeTranslation = {};
        Object.entries(SharedPropertyCategories).forEach(([category, translations]) => { translations.en.forEach((enType, index) => {
		...
		const createCityCheckboxes = (category, cities) => `
			<div>
				<div class="collapsible city-category" onclick="toggleCollapse(this)">
					${category}
				</div>
				<div class="collapse-content">
		...
		formContainer.addEventListener("submit", (event) => {
			...
			const selectedPropertyCategories = isEnglish ? selectedPropertyCategoriesRaw.map(cat => PropertyTypeTranslation[cat] || cat) : selectedPropertyCategoriesRaw;
			...
			const indoorParkingCars = indoorParking === "Yes" ? parseInt(formContainer.querySelector("#garage-cars").value || 0, 10) : 0;
			const swimmingPool = formContainer.querySelector("#swimming-pool").checked ? "Yes" : "No";
