...
.time-slot::before {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(120deg, transparent, #d7dbeb, transparent);
    ...
    const CancellationCalendarExtension = {
      ...
      render: async ({ trace, element }) => {
        ...
        style.textContent = `
          ...
          .extension-container {
            box-shadow: 0 10px 25px #003da526;
            border-radius: 16px;
            overflow: hidden;
            background: #fff;
            color: #333;
