...
Object.values(SharedPropertyTypes).forEach(category => {
   category.en.forEach((enType, index) => {
      const frType = category.fr[index];
      PropertyTypeTranslation[enType] = frType;
   });
});
...
function generateAirtableFormula(input) {
   ...
   if (typeHouse.length > 0) {
      const typeConditions = typeHouse.map(type => `FIND("${type}", {Type})`);
